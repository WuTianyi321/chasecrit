...
## 2026-02-07 22:00+ SOC-v3 larger scan expansion (requested more data)

### What was added

To address insufficient sample size, ran a much larger SOC-v3 scan plus focused 200-seed validation.

### Broad scan

- root: `runs/soc_v3_scan_large_20260207/`
- manifest: `runs/soc_v3_scan_large_20260207/manifest.csv`
- variants: `14` total
  - `baseline`
  - `soc_v3_no_entropy`
  - `soc_v3_eg{0.2,0.6,1.0,1.4}_th{0.28,0.32,0.36}`
- setup:
  - `speed_ratio={1.0,1.1,1.2,1.3,1.4}`
  - `seeds=0..79` (`80`)
  - `steps=600`, `jobs=8`
- total runs: `5600`
...
- `doc/results_20260207_soc_v3_large_scan/soc_v3_large_runs.csv`
...
- `doc/results_20260207_soc_v3_large_scan/soc_v3_large_best_by_speed_ratio.csv`
- figs:
  - `baseline_vs_best_nonbaseline_safe.png`
  - `delta_safe_vs_delta_pred_entropy.png`
  - `heatmap_safe.png`
  - `heatmap_delta_safe.png`

Broad finding:

- best non-baseline variants are high entropy-gain (`eg=1.4`) groups with branch ratio near 1, but still slightly below baseline on average safe.

### Focused 200-seed extension

- root: `runs/soc_v3_scan_large_20260207_ext/`
- manifest: `runs/soc_v3_scan_large_20260207_ext/manifest.csv`
- variants:
  - `baseline`
  - `soc_v3_eg14_th036`
  - `soc_v3_eg14_th028`
- added seeds: `80..199`; merged with prior seeds `0..79` => `200 seeds`.

200-seed outputs:

- `doc/results_20260207_soc_v3_large_scan/focus_200seeds/runs_200seeds.csv`
- `doc/results_20260207_soc_v3_large_scan/focus_200seeds/summary_200seeds.csv`
- `doc/results_20260207_soc_v3_large_scan/focus_200seeds/paired_ci_200seeds.csv`
- fig: `doc/results_20260207_soc_v3_large_scan/focus_200seeds/figs/safe_compare_200seeds_focus.png`

Key 200-seed paired-CI finding:

- `sr=1.3`: both top SOC-v3 variants significantly below baseline (`delta_safe` CI fully negative).
- `sr=1.4`: `eg14_th036` significantly below baseline; `eg14_th028` negative with CI crossing 0.
- `sr<=1.2`: differences are smaller and mostly inconclusive (CI crosses 0).
- both top variants consistently increase `soc_pred_entropy_var` vs baseline with very tight CI.
...
- Increasing predictive-entropy variance (paper-aligned signal) is reproducibly achieved, but does not guarantee better safe performance under stronger pressure layers in current SOC-v3 control family.

### Remote sync update (2026-02-07)

- Commit `2234aa3` (SOC-v3 large scan + 200-seed focus) has been pushed to `origin/main`.
- Push result: `e064ad4..2234aa3  main -> main`.
- Repository state after push: clean working tree on `main`.

### Predictive-dependency scan (2026-02-07, expanded)

Objective:

- Test whether stronger pursuer predictive dependency makes near-critical proxies (especially `chi_local`) more predictive of `safe`.

Code changes (backward-compatible defaults):

- Added pursuer config knobs:
  - `pursuers.intercept_gain` (default `1.0`)
  - `pursuers.intercept_tmax` (default `0.0`, unlimited horizon)
- Wiring:
  - `src/chasecrit/config.py` (`PursuerConfig` fields)
  - `src/chasecrit/sim.py` (pass-through to `p1_intercept`)
  - `src/chasecrit/policies.py` (`pursuer_step_p1_intercept` blend + horizon limit)
- Added tests:
  - `tests/test_pursuer_policies.py`
    - gain=0 degenerates to nearest-chase direction.
    - short horizon reduces predictive lead.
- Bug fixed during implementation:
  - local variable shadowing in quadratic-root branch (`tmax_root` rename) to ensure `intercept_tmax` is effective.

Validation:

- `uv run pytest tests/test_pursuer_policies.py -q` -> `4 passed`
- `uv run pytest -q` -> `18 passed`
- `uv run python -m compileall -q src` -> success.

Experiment configs:

- `configs/predictive_dependency_scan_20260207/`
  - `p0_nearest_share_noise0.toml`
  - `p1_full_share_noise0.toml` (`gain=1.0, tmax=0`)
  - `p1_gain05_share_noise0.toml` (`gain=0.5, tmax=0`)
  - `p1_tmax2_share_noise0.toml` (`gain=1.0, tmax=2`)

Large run batch:

- Root: `runs/predictive_dependency_scan_20260207/`
- Manifest: `runs/predictive_dependency_scan_20260207/manifest.csv`
- Per variant sweep size: `21 w_align × 5 speed_ratio × 60 seeds = 6300 runs`
- Total: `25200 runs`
- Fixed settings:
  - `w_align=0..1 step 0.05`
  - `speed_ratio={1.0,1.1,1.2,1.3,1.4}`
  - `steps=600`, `jobs=8`, no timeseries/events.

Reader-facing outputs:

- `doc/results_20260207_predictive_dependency_scan_60seeds/report.md`
- data tables:
  - `runs_merged.csv`
  - `group_summary.csv`
...
- figures:
  - `safe_vs_w_align_by_policy_sr.png`
  - `chi_local_vs_w_align_by_policy_sr.png`
  - `corr_safe_chi_local_by_policy_sr.png`
  - `scatter_safe_vs_chi_local_per_run_by_policy_sr.png`
  - `best_safe_by_policy_sr.png`
  - `delta_best_safe_vs_p0_by_policy_sr.png`

Main findings (task-internal):

- All three predictive variants (`p1_*`) outperform `p0_nearest` in best attainable `safe` for each tested speed layer.
- `chi_local` is consistently more useful than global `chi` for explaining `safe` variation under predictive pursuers.
- As predictive dependency strengthens, `corr(safe, chi_local)` and highQ-lowQ gain by `chi_local` are generally larger.
- At higher pressure (`sr>=1.2`), best `w_align` shifts lower.
