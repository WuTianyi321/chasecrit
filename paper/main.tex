...
\begin{table}[htbp]
\centering
\caption{噪声路线关键统计（全部纳入）}
\label{tab:noise_all}
\begin{tabular}{cccccc}
\toprule
设置 & seeds & \(\arg\max \mathrm{safe}\) & \(\arg\max \chi\) & \(\mathrm{corr}(\mathrm{safe},\chi)\) & Spearman \(\rho_S\) \\
\midrule
Task, \(w_{\text{align}}=0.6\) & 100 & \(0.0\) & \(0.0\) & 0.814 & 0.965 \\
Task, \(w_{\text{align}}=1.0\) & 100 & \(0.2\) & \(0.0\) & 0.824 & 0.965 \\
Phase (no pursuer/zone) & 100 & N/A & \(1.8\) & N/A & N/A \\
\bottomrule
\end{tabular}
\end{table}
...
\]
并且 \(\mathrm{safe}\) 峰值在 \(\text{noise}=0.2\)，\(\chi\) 峰值在 \(\text{noise}=0.0\)。这说明任务内低噪声区同时给出高生存率与高 \(\chi\)，但与 phase 峰值 \(\text{noise}\approx 1.8\) 并不一致。
