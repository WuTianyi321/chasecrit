...
\)
（并辅以 \(\chi_{\text{local}}\)、\(\tau_{P,\mathrm{AR1}}\)、\(\xi_{\text{fluct}}\)）比较“相对更临界”的参数区。实验基于容量受限、缓慢移动、多安全区机制，覆盖 \(w_{\text{align}}\) 主线扫描、追捕压力对比、噪声路线对照（含 phase 识别与任务噪声扫描）。结果显示：在中等压力区间（\(v_p/v_e \approx 1.0\sim1.3\)）内，生存率与 \(\chi\) 呈稳定正相关；在高压力（\(v_p/v_e=1.4\)）下该关系显著减弱甚至失效。噪声路线中，任务设置 \(w_{\text{align}}=1.0\) 的 \(\mathrm{corr}(\mathrm{safe},\chi)\) 达到 0.824（Spearman 0.965），但 phase 峰值噪声并不对应任务最优。结论表明近临界优势是受外场与任务机制共同调制的条件性现象。
\end{abstract}
...
\item 噪声路线：phase 识别（seeds=100, steps=1200）与任务噪声扫描（\(w_{\text{align}}\in\{0.6,1.0\}\), \(v_p/v_e=1.1\), seeds=100）。
\end{itemize}
...
\subsection{噪声路线：将“好结果”完整纳入主文}
用户指出的强正相关结果对应任务噪声扫描设置（\(w_{\text{align}}=1.0, v_p/v_e=1.1, \text{seeds}=100\)）。本节将 phase 识别与两个任务噪声扫描（\(w_{\text{align}}=0.6,1.0\)）统一汇总。

\begin{table}[htbp]
\centering
\caption{噪声路线关键统计（全部纳入，明确峰值位置与峰值数值）}
\label{tab:noise_all}
\begin{tabular}{ccccccc}
\toprule
设置 & seeds & noise@safe\(_{\max}\) & safe\(_{\max}\) & noise@\(\chi_{\max}\) & \(\chi_{\max}\) & \(\mathrm{corr}(\mathrm{safe},\chi)\) \\
\midrule
Task, \(w_{\text{align}}=0.6\) & 100 & 0.00 & 0.3567 & 0.00 & 4.2812 & 0.814 \\
Task, \(w_{\text{align}}=1.0\) & 100 & 0.20 & 0.3387 & 0.00 & 4.2378 & 0.824 \\
Phase（无追捕/无安全区） & 100 & -- & -- & 1.80 & 13.2085 & -- \\
\bottomrule
\end{tabular}
\end{table}

其中，\(w_{\text{align}}=1.0\) 的“强正相关”结果为：
\[
\mathrm{corr}_{\mathrm{Pearson}}(\mathrm{safe},\chi)=0.824,\quad
\rho_S(\mathrm{safe},\chi)=0.965,
\]
并且 \(\mathrm{safe}\) 峰值在 \(\text{noise}=0.20\)，\(\chi\) 峰值在 \(\text{noise}=0.00\)。这说明任务内低噪声区同时给出高生存率与高 \(\chi\)，但与 phase 峰值 \(\text{noise}\approx 1.8\) 并不一致。

\begin{figure}[htbp]
\centering
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{../doc/results_20260206_task_noise_w10_sr11_100seeds/figs/safe_vs_noise.png}
\caption{Task, \(w_{\text{align}}=1.0\): \(\mathrm{safe}\)-noise}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\includegraphics[width=\textwidth]{../doc/results_20260206_task_noise_w10_sr11_100seeds/figs/scatter_safe_vs_chi.png}
\caption{Task, \(w_{\text{align}}=1.0\): \(\mathrm{safe}\)-\(\chi\)}
\end{subfigure}
\caption{用户指出的强相关结果：任务噪声扫描 \(w_{\text{align}}=1.0\)}
\label{fig:w10_noise}
\end{figure}
...
\begin{itemize}
\item “任务内更临界（统计意义）是否更好”在部分压力区成立（含 \(w_{\text{align}}=1.0\) 噪声扫描中的强正相关结果）；
...
\appendix
\section{完整实验结果索引（尽量全纳入）}
为避免“只展示部分结果”造成结论偏差，本附录列出当前仓库中的主要实验结果目录及用途。本文正文图表来自其中的关键数据集，其余结果作为补充证据可直接复核。

\begin{longtable}{p{3.5cm}p{1.8cm}p{1.8cm}p{7.3cm}}
\toprule
结果目录 & seeds & steps & 说明 \\
\midrule
\endfirsthead
...
\midrule
\endhead
R1: walign\_task\_200 & 200 & 600 & 对应目录：\texttt{results\_20260206\_walign\_task\_internal\_200seeds}；\(w_{\text{align}}\) 主扫描（\(v_p/v_e=1.0\sim1.2\)），正文主结果之一。 \\
R2: pressure\_sr14\_240 & 120/240 & 600 & 对应目录：\texttt{results\_20260206\_walign\_pressure\_091314\_sr14\_240seeds}；压力对比（0.9/1.3/1.4，1.4 扩样），正文主结果之一。 \\
R3: walign\_task\_80 & 80 & 600 & 对应目录：\texttt{results\_20260206\_walign\_task\_internal\_80seeds}；主扫描早期版本，用于稳定性判断与补样依据。 \\
R4: pressure\_120 & 120 & 600 & 对应目录：\texttt{results\_20260206\_walign\_pressure\_091314\_120seeds}；压力对比早期版本（后续由 R2 覆盖）。 \\
R5: noise\_task\_w10 & 100 & 600 & 对应目录：\texttt{results\_20260206\_task\_noise\_w10\_sr11\_100seeds}；噪声路线（\(w_{\text{align}}=1.0\)），强正相关 \(r=0.824\)。 \\
R6: noise\_task\_w06 & 100 & 600 & 对应目录：\texttt{results\_20260206\_task\_noise\_w06\_sr11\_100seeds}；噪声路线对照组。 \\
R7: noise\_phase\_100 & 100 & 1200 & 对应目录：\texttt{results\_20260206\_phase\_noise\_100seeds\_steps1200}；phase 识别（无追捕/无安全区）对照。 \\
R8: fixedNp\_100 & 100 & 600 & 对应目录：\texttt{results\_20260206\_fixedNp\_sr11\_sr12\_100seeds}；固定追捕者扫描历史高样本结果。 \\
R9: fixedNp\_50\_v2 & 50 & 600 & 对应目录：\texttt{results\_20260205\_fixedNp\_scan\_50seeds\_v2}；早期固定追捕者扫描（含 \(\chi_{\text{local}}, \xi\)）。 \\
R10: fixedNp\_exp & 15 & 600 & 对应目录：\texttt{results\_20260205\_fixedNp\_scan\_expanded}；扩展网格早期版本。 \\
R11: fixedNp\_pilot & 10 & 600 & 对应目录：\texttt{results\_20260205\_fixedNp\_scan}；初版试跑结果。 \\
R12: noise\_task\_early & 15 & 600 & 对应目录：\texttt{results\_20260205\_task\_noise\_walign1}；噪声任务扫描早期版本。 \\
R13: noise\_phase\_early & 20 & 1200 & 对应目录：\texttt{results\_20260205\_phase\_noise\_wider}；phase 识别早期扩展版本。 \\
\bottomrule
\end{longtable}

\section{补充说明：关于“表 2 全是 0”的疑问}
表~\ref{tab:noise_all} 已按原始 \texttt{group\_summary.csv} 重算并明确写出峰值位置与峰值值：
\begin{itemize}
\item \(w_{\text{align}}=1.0\) 时，noise@safe\(_{\max}=0.20\)，不是 0；
\item \(w_{\text{align}}=1.0\) 时，noise@\(\chi_{\max}=0.00\)；
\item phase 行由于无安全区，safe 指标不适用，已明确标为 ``--''。
\end{itemize}
...
\item \texttt{doc/results\_20260206\_task\_noise\_w10\_sr11\_100seeds}
\item \texttt{doc/results\_20260206\_task\_noise\_w06\_sr11\_100seeds}
\item \texttt{doc/results\_20260206\_phase\_noise\_100seeds\_steps1200}
