...
为更直接对照“更接近临界（χ 更大）是否带来更好逃脱”，下表给出每个 `v_p/v_e` 下的最优 `w_align`（按 `safe_frac`）、以及 χ 峰值点的 `w_align` 与对应 `safe_frac`（均为格点聚合均值）：

| `v_p/v_e` | `w_safe` | `safe_frac(w_safe)` | `w_χ` | `χ(w_χ)` | `safe_frac(w_χ)` | gap = `safe_frac(w_safe)-safe_frac(w_χ)` |
|---:|---:|---:|---:|---:|---:|---:|
| 1.00 | 0.65 | 0.4214 | 0.55 | 5.9983 | 0.3620 | 0.0594 |
| 1.05 | 0.50 | 0.3974 | 0.70 | 4.9865 | 0.3469 | 0.0505 |
| 1.10 | 0.65 | 0.3719 | 0.70 | 5.8642 | 0.2859 | 0.0859 |
| 1.15 | 0.70 | 0.3797 | 0.20 | 5.0652 | 0.3318 | 0.0479 |
| 1.20 | 0.65 | 0.3422 | 0.70 | 4.2466 | 0.2854 | 0.0568 |

该对照显示：在本轮设置与 χ 定义下，χ 峰值点并不等同于任务成功率最优点；但在多数强度下 `w_safe` 与 `w_χ` 的差距不大（除 `v_p/v_e=1.15` 外），提示仍可能存在“高涨落区附近性能较好”的趋势，需要更严格的临界性定位与稳健性检验来确认。
