...
- **任务内判定优先**：在追逃任务结论中，默认以同一任务设置内观测到的统计指标进行“更接近临界”的相对判定（例如 `chi`、`chi_local`、`tau_P_ar1`、`xi_fluct`），不将外部相变识别设置中的峰值位置直接当作任务最优先验。
- **对抗解释约束**：在追逃任务中，必须区分“自主临界”与“受迫有序”。若出现“临界性代理变大且性能变好”，需说明这是任务内相关性还是相变临界点迁移，并结合可预测性/追捕强度给出解释边界。
- **单参数清晰性优先（下一阶段）**：当研究目标是检验“听邻居程度”的单因子作用时，优先使用 `evaders.align_control_mode="share"` 且 `angle_noise=0`；此时扫描 `w_align∈[0,1]` 代表“对齐项在总方向中的占比”，其余权重保持固定。
...
- **科学论文语体**：正文使用客观、第三人称、可检验表述；避免“好结果”“看起来”等口语化评价。
- **正文完整性**：阶段性论文版本应在正文（含附录）覆盖全部已完成实验批次，至少给出统一编号、设置摘要与关键统计/图表映射。
- **避免路径叙述**：论文正文不以数据路径描述结论；实验可追溯性通过实验编号、图表与表格实现（源码中的 `\includegraphics` 路径不视为叙述文本）。
- **方法可复现性**：方法章节需明确给出动力学更新规则（位置/速度更新、边界处理、捕获判定、安全区移动与刷新、随机项分布），并尽量用公式化写法使读者可据此复现实现。
...
- **大扫参前置性能基线**：在发起大规模实验前，先完成一次小规模测速与行为一致性验证（例如 `bench-speed` + 对照测试），并将结果写入 `doc/`，再开始长扫。
...
- 主动 Git 管理：每完成一个可复现的阶段性里程碑（代码 + 测试 + 文档/结果）后，应主动完成一次 commit，并在无阻塞条件下主动 push 到远端；不依赖用户额外提醒。
- 若暂不适合 push（例如远端冲突、凭证问题、CI 红灯），需在对话与 `memory/YYYY-MM-DD.md` 中明确记录阻塞原因、当前分支状态和下一步处置命令。
- commit 前必须先检查工作区，避免将无关临时文件混入提交；若存在跨任务改动，按主题拆分提交并在提交信息中标注范围。
...
- 使用方式：
  ...
  - `MEMORY.md` 以增量维护为主；当结构老化、检索困难或出现重要路线调整时，应进行必要的重构式整理（重排章节、合并冗余、替换过时内容），确保其始终可检索且可执行。
  - 对关键假说与争议结论（例如“某类临界性是否提升任务性能”）应记录“支持证据 / 反例证据 / 适用场景 / 当前判定状态”，避免后续对话只保留结论而丢失依据。

重要：`memory/` 是唯一可信的“长期记忆”。在新对话中，除 `memory/` 外的上下文不保证存在或准确。

- 任何可能在后续用到的信息都应写入 `memory/`（例如：当前做到哪里、最近的实验 ID 与输出路径、关键命令、结论、下一步计划、已知问题、性能瓶颈与优化点、与 GitHub 同步状态）。
- 避免“只在对话里说但不落盘”：对项目推进有影响的决策与进展必须同步到当天的 `memory/YYYY-MM-DD.md`，并在需要时提炼到 `memory/MEMORY.md`。
